-- =====================================================
-- WORKING BOOKINGS MIGRATION - PROPERLY TESTED
-- =====================================================

BEGIN;

-- =====================================================
-- FIRST: Get some real student IDs to work with
-- =====================================================
DO $$ 
DECLARE
    student1 uuid;
...
BEGIN
    ...
    SELECT id INTO student3 FROM students ORDER BY created_at DESC LIMIT 1 OFFSET 2;
    SELECT id INTO student4 FROM students ORDER BY created_at DESC LIMIT 1 OFFSET 3;
    SELECT id INTO student5 FROM students ORDER BY created_at DESC LIMIT 1 OFFSET 4;
    ...
    SELECT 
        ...
        80.00, 80.00,
        'My essay about overcoming challenges in starting a nonprofit...',
        'Brown supplemental - review for authenticity',
        NOW() + INTERVAL '2 days',
        false
    ...
    SELECT 
        ...
        80.00, 80.00,
        5,
        'Priya was amazing! Her feedback completely transformed my essay.',
        NOW() - INTERVAL '5 days',
        NOW() - INTERVAL '7 days',
        false
    WHERE EXISTS (SELECT 1 FROM services WHERE id = '8eaf2101-be9f-43f9-be26-01e563ea1cf2');
    ...
    -- =====================================================
    -- COMPLETED BOOKINGS - UNRATED (for rating prompt)
    -- =====================================================
    ...
    -- =====================================================
    -- WAITLISTS (only add if students exist)
    -- =====================================================
...
-- =====================================================
-- GROUP SESSIONS (Separate handling)
-- =====================================================

-- First enable group sessions for some services
UPDATE services 
SET allows_group_sessions = true, max_group_size = 8
WHERE id IN (
    SELECT id FROM services
    WHERE title ILIKE '%essay%' 
    AND consultant_id = '639b801b-e0cd-4231-9139-0eda9cb7e39a'
    AND allows_group_sessions = false
    LIMIT 1
);

-- Create a group session owned by the FIRST STUDENT who creates it
INSERT INTO bookings (
    student_id, consultant_id, service_id, scheduled_at, status,
...
SELECT 
    '338feabe-39f2-492a-93e4-ee746bf727b7'::uuid, -- Main student creates the group
    s.consultant_id,
    s.id,
    ...
    'Group Essay Workshop: Personal Statement Mastery',
...
WHERE b.is_group_session = true
    ...
    AND b.student_id = '338feabe-39f2-492a-93e4-ee746bf727b7'
ON CONFLICT (booking_id, student_id) DO NOTHING;

-- Add other students as participants
INSERT INTO group_session_participants (booking_id, student_id, joined_at)
SELECT 
    b.id,
    s.id,
...
CROSS JOIN (
    ...
    WHERE id != '338feabe-39f2-492a-93e4-ee746bf727b7'
...
-- Check bookings for main student
SELECT 
    b.status,
    b.scheduled_at,
