-- =====================================================
-- MOCK BOOKINGS DATA MIGRATION FOR PROOFR
-- =====================================================
...
FROM (
  ...
  SELECT 
    ...
    (SELECT id FROM services WHERE consultant_id = '6e4ebdab-dad2-47ed-80b0-a444583756d5' AND service_type = 'essay_review' LIMIT 1) as service_id,
    'essay_review' as service_type,
    NOW() + INTERVAL '1 day' + TIME '14:00:00' as scheduled_at,
    60 as duration_minutes,
    'confirmed' as status,
    120.00 as base_price,
    120.00 as final_price,
    2.40 as credits_earned,
  ...
  SELECT 
    ...
    (SELECT id FROM services WHERE consultant_id = 'cf427c87-f260-49fb-9ced-d62cd6f78318' AND service_type = 'mock_interview' LIMIT 1),
    'mock_interview',
...
SELECT
  ...
  (SELECT id FROM services WHERE consultant_id = 'f3189645-f5b7-46ec-858e-45334978f713' AND service_type = 'essay_review' LIMIT 1) as service_id,
  'essay_review',
  ...
  false,
  NOW() - INTERVAL '2 days'
WHERE (SELECT id FROM services WHERE consultant_id = 'f3189645-f5b7-46ec-858e-45334978f713' AND service_type = 'essay_review' LIMIT 1) IS NOT NULL
ON CONFLICT (id) DO NOTHING;
...
) AS completed_bookings(student_id, consultant_id, service_id, service_type, scheduled_at, duration_minutes, status, base_price, final_price, credits_earned, rating, review_text, reviewed_at, delivered_at, is_group_session, created_at)
...
SELECT * FROM (
  ...
  (
    ...
    (SELECT id FROM services WHERE consultant_id = '1684c912-8fc3-4d46-862a-c5e9177530a4' AND service_type = 'essay_review' LIMIT 1),
    'essay_review',
    NOW() - INTERVAL '3 days',
    0,
    'completed',
  ...
  (
    ...
    (SELECT id FROM services WHERE consultant_id = '7cefd294-6775-497d-a13a-373eea813373' AND service_type = 'test_prep' LIMIT 1),
    'test_prep',
    NOW() - INTERVAL '4 days',
    60,
    'completed',
...
) AS unrated_bookings(student_id, consultant_id, service_id, service_type, scheduled_at, duration_minutes, status, base_price, final_price, credits_earned, delivered_at, is_group_session, created_at)
WHERE service_id IS NOT NULL
ON CONFLICT (id) DO NOTHING;
...
SET 
  ...
  max_group_size = 6
WHERE service_type IN ('essay_review', 'test_prep', 'college_list_strategy')
...
-- Create available group sessions
INSERT INTO bookings (
  student_id,
  consultant_id,
  service_id,
...
) AS group_sessions(student_id, consultant_id, service_id, service_type, scheduled_at, duration_minutes, status, base_price, final_price, credits_earned, is_group_session, max_participants, current_participants, prompt_text, meeting_link, created_at)
WHERE service_id IS NOT NULL
ON CONFLICT (id) DO NOTHING;
...
WHERE b.is_group_session = true 
  AND b.service_type = 'group_essay_workshop'
...
FROM bookings b
WHERE b.is_group_session = true 
  AND b.service_type = 'sat_math_bootcamp'
  AND b.scheduled_at > NOW()
LIMIT 1
ON CONFLICT (booking_id, student_id) DO NOTHING;
...
-- Check active bookings for student
-- SELECT 
--   b.id,
--   b.status,
--   b.scheduled_at,
