-- Migration 010: Booking Requests System
-- Description: Creates tables for pre-payment booking request workflow
-- Author: Proofr Team
-- Date: 2025-01-23
...
CREATE TABLE public.booking_file_uploads (
  ...
  scheduled_deletion_date DATE GENERATED ALWAYS AS ((created_at + INTERVAL '30 days')::DATE) STORED,
...
-- Create storage bucket for file uploads (to be run in Supabase dashboard)
...
CREATE POLICY "Users can view request inputs" ON public.booking_text_inputs
  FOR SELECT USING (
    EXISTS (
      ...
      AND (auth.uid() = br.student_id::text OR auth.uid() = br.consultant_id::text)
    )
  );
...
CREATE POLICY "Students can manage own inputs" ON public.booking_text_inputs
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.booking_requests br
      WHERE br.id = request_id
      AND auth.uid() = br.student_id::text
      AND br.status = 'draft'
    )
  );
