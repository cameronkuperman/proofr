-- =====================================================
-- MOCK BOOKINGS DATA MIGRATION FOR PROOFR (FIXED)
-- =====================================================
...
) AS completed_bookings(student_id, consultant_id, service_id, scheduled_at, status, base_price, final_price, credits_earned, rating, review_text, reviewed_at, delivered_at, is_group_session, created_at)
...
) AS unrated_bookings(student_id, consultant_id, service_id, scheduled_at, status, base_price, final_price, credits_earned, delivered_at, is_group_session, created_at)
WHERE service_id IS NOT NULL
ON CONFLICT (id) DO NOTHING;
...
) AS group_sessions(student_id, consultant_id, service_id, scheduled_at, status, base_price, final_price, credits_earned, is_group_session, max_participants, current_participants, prompt_text, meeting_link, created_at)
WHERE service_id IS NOT NULL
ON CONFLICT (id) DO NOTHING;
...
-- Check active bookings for main student
-- SELECT 
--   b.id,
--   b.status,
--   b.scheduled_at,
